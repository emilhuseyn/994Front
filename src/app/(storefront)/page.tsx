...
const DEFAULT_SLIDE: ApiSlider = {
  ...
  subtitleAz:
    '150 ₼-dən yuxarı sifarişlər üçün Bakı üzrə pulsuz çatdırılma və ya mağazadan götürmə.',
  subtitleRu:
    'Для заказов свыше 150 ₼ бесплатная доставка по Баку или самовывоз из магазина.',
  subtitleEn: 'Free delivery in Baku or in-store pickup for orders over 150 ₼.',
  imageUrl:
...
/**
 * Times the phrase is repeated inside one half of the ticker. Six keeps a
 * half wider than a 4K viewport in every locale; below that, wide screens
 * see empty space trailing the second half at the end of each cycle.
 */
const TICKER_REPEAT = 6;

/**
 * International-delivery ticker — a scrolling strip between the category
 * tiles and the product grids.
 *
 * The animation shifts a two-half container by -50%, so each half must be at
 * least as wide as the viewport or a gap would trail the second half. The
 * phrase is repeated TICKER_REPEAT times per half to guarantee that on wide
 * screens and in every locale.
 *
 * The whole strip is aria-hidden and paired with one sr-only sentence: the
 * visual repetition is decorative, and a screen reader should hear the fact
...
function DeliveryTicker() {
  ...
  return (
    <section className="overflow-hidden bg-neutral-900 py-3 text-white">
      <p className="sr-only">
        {t('home.delivery.title')}. {t('home.delivery.body')}
      </p>
      <div
        aria-hidden
...
function TickerRow() {
  ...
  return (
    <div className="flex shrink-0 items-center whitespace-nowrap text-[11px] uppercase tracking-[0.15em]">
      {Array.from({ length: TICKER_REPEAT }).map((_, i) => (
        <Fragment key={i}>
          <span className="px-5 font-semibold">{t('home.delivery.title')}</span>
          <span className="text-white/40">✳</span>
          <span className="px-5 text-white/60">{t('home.delivery.body')}</span>
          <span className="text-white/40">✳</span>
        </Fragment>
      ))}
    </div>
